...
const projects: Project[] = [
  {
    slug: "trybl",
    title: "Trybl Discovery Service",
    description:
      "An ML-powered microservice that generates, ranks, and deduplicates personalized activity recommendations for a social discovery platform.",
    longDescription:
      "I built Trybl's discovery service: a Python microservice that turns a user's location, availability, preferences, budget, and interaction history into a ranked set of activities they are likely to enjoy. FastAPI exposes the recommendation workflow, GPT-4o generates context-aware candidates, PostgreSQL caches results by community, and Model2Vec embeddings keep the feed fresh by identifying semantically similar events before they reach the user. The service integrates with Trybl's Go backend through authenticated APIs and is designed to stay reliable under concurrent batch workloads.",
    ...
    highlights: [
      "Built a personalized discovery pipeline around location, availability, preferences, budget, and interaction history",
      "Used GPT-4o to generate context-aware activity candidates, then ranked and cached recommendations by community",
      "Implemented semantic deduplication with Model2Vec embeddings so users do not repeatedly see near-identical events",
      "Integrated the Python service with Trybl's Go backend using authenticated APIs, batch processing, concurrency locks, timeouts, retries, and exponential backoff",
    ],
    caseStudy: {
      context:
        "Trybl helps people discover things to do together, but useful recommendations require more than a generic list of nearby events. I built the discovery service to combine each user's real-world constraints and preferences with their interaction history, producing activity suggestions that are timely, relevant, and varied enough to keep exploring.",
      constraints: [
        "Recommendations must respect multiple signals at once: location, time, interests, budget, and past behavior.",
        "Generative models can produce superficially different wording for essentially the same activity, making exact-match deduplication ineffective.",
        "Generation is comparatively expensive, so the service needed to reuse work across members of the same community without serving stale or irrelevant results.",
        "The Python service and Go backend have independent workloads and failure modes, so their API boundary needed authentication and defensive network behavior.",
      ],
      decisions: [
        {
          decision: "Isolate recommendation logic in a Python + FastAPI microservice",
          why: "Python provides the strongest ecosystem for embeddings and model orchestration, while FastAPI gives the Go backend a small, typed HTTP boundary.",
          tradeoff: "A separate service adds deployment and network complexity, so the interface and failure behavior have to be explicit.",
        },
        {
          decision: "Generate candidates with GPT-4o from structured user context",
          why: "A generative model can synthesize several constraints into specific, context-aware ideas instead of selecting only from a fixed catalog.",
          tradeoff: "Model output is slower and less deterministic than conventional queries, so generation needs validation, caching, and bounded execution.",
        },
        {
          decision: "Use Model2Vec embeddings for semantic deduplication",
          why: "Vector similarity catches recommendations that describe the same underlying activity with different wording.",
          tradeoff: "Similarity thresholds require calibration: too strict removes useful variety, while too loose lets repetitive suggestions through.",
        },
        {
          decision: "Cache generated recommendations in PostgreSQL by community",
          why: "People in the same community share relevant local context, allowing the service to reuse quality candidates and reduce repeated model calls.",
          tradeoff: "Community-level reuse has to be balanced with user-level ranking so shared candidates still feel personal.",
        },
        {
          decision: "Harden the Go integration for batch and concurrent work",
          why: "Authenticated requests, concurrency locks, timeouts, retries, and exponential backoff prevent transient failures or duplicate jobs from destabilizing discovery.",
          tradeoff: "Retries improve resilience but can amplify load, so attempts must be bounded and paired with backoff and locking.",
        },
      ],
      results: [
        { metric: "Personalization signals", value: "5", note: "location, availability, preferences, budget, and history" },
        { metric: "Generation", value: "GPT-4o", note: "context-aware activity candidates" },
        { metric: "Deduplication", value: "Semantic", note: "Model2Vec embedding similarity" },
        { metric: "Backend integration", value: "Production-ready", note: "auth, batching, locks, timeouts, retries, and backoff" },
      ],
      nextTime:
        "The next iteration would make recommendation quality measurable end to end: version prompts and ranking logic, log why each candidate was selected or rejected, and run controlled experiments on relevance, diversity, and downstream engagement.",
      architecture: {
        tiers: [
          {
            label: "Inputs",
            nodes: [
              { name: "User context", detail: "location · availability · preferences · budget" },
              { name: "Interaction history", detail: "behavior signals for personalized ranking" },
            ],
          },
          {
            label: "Discovery service",
            nodes: [
              { name: "FastAPI orchestration", detail: "authenticated requests · batch processing" },
              { name: "GPT-4o generation", detail: "context-aware activity candidates" },
              { name: "Model2Vec filtering", detail: "semantic deduplication" },
            ],
          },
          {
            label: "Delivery",
            nodes: [
              { name: "PostgreSQL cache", detail: "recommendations grouped by community" },
              { name: "Trybl Go backend", detail: "ranked recommendations returned to the product" },
            ],
          },
        ],
        notes: [
          "Concurrency locks prevent overlapping batch work from producing duplicate or conflicting results.",
          "Timeouts, bounded retries, and exponential backoff isolate transient failures at the service boundary.",
        ],
      },
    },
  },
